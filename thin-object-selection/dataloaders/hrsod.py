""" Dataset from HRSOD.
Reference:
[1] Zeng et al. Towards high-resolution salient object detection. ICCV 2017.
[2] https://arxiv.org/abs/1908.07274
"""
import os
import sys
import cv2
...
class HRSOD(data.Dataset):
    def __init__(
        self,
        split='test',
        root=Path.db_root_dir('hrsod'),
        transform=None,
        area_thres=0,
        retname=True):
        ...
        if (not self._check_preprocess()):
            print('Preprocessing HRSOD dataset, this will take long, but it will be done only once.')
            self._preprocess()
    ...
    def _make_img_gt_pair(self, index):
        ...
        # Reference: https://github.com/python-pillow/Pillow/issues/4703#issuecomment-645219973
        _img = Image.open(self.imgs[_img_ii])
        _img = np.float32(ImageOps.exif_transpose(_img))
...
if __name__ == '__main__':
    ...
    dataset = HRSOD(split='test', transform=transform)
